...
class DetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String billNumber = getBillNumber(qrCode);

    // Parse the QR code and extract information
    List<Medicine> medicines = parseQRCode(qrCode);

    // Calculate the total amount and total cost of all medicines
    int totalAmount = medicines.fold(0, (sum, medicine) => sum + medicine.amount);
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          for (int index = 0; index < medicines.length; index++)
            Padding(
              ...
              child: ListTile(
                ...
                trailing: Text('Rs.${medicines[index].amount * medicines[index].cost}'),
          ...
          ElevatedButton(onPressed: () {}, child: Text('Get medicine', style: TextStyle(color: Colors.black),),style: ElevatedButton.styleFrom(backgroundColor: Colors.lightGreenAccent,),)
        ],
      ),
    );
  }
